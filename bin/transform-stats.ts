    ...
    for (const { name, id: moduleId, chunks: moduleChunks } of modules) {
      ...
      if (!name.match(/node_modules/) && !name.match(/\(webpack\)/)) {
        ...
        if (
          ...
          const currentParentIdArray = statsObj.modulesById[moduleId].parentFiles
          const parentFiles = []
        ...
        if (
          ...
          const currentSiblingIdArray = statsObj.modulesById[moduleId].siblingFiles
          const siblingFiles = []
          for (const { id, files } of chunks) {
